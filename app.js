...
const App = {
    ...
    renderSpecialistSummary() {
        ...
        this._sp().forEach(sp => { const sub = sp.subject || sp.name || ''; if(!sub || !sts[sub]) return; if((sp.hiddenWeeks || []).includes(this.state.currentWeek)) return; this.days.forEach(d => { for(let p=0; p<this.state.config.periods[d]; p++){ const raw = sp.data[d] && sp.data[d][p]; if(!raw) continue; String(raw).split(/[,\s]+/).map(v => parseInt(v.trim())).filter(n => !isNaN(n) && n > 0).forEach(cN => { if(sts[sub][cN] !== undefined) sts[sub][cN]++; }); } }); });
